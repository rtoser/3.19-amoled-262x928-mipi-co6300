...
**Two software draw threads** (`CONFIG_LV_OS_FREERTOS=y` + `CONFIG_LV_DRAW_SW_DRAW_UNIT_CNT=2`, Espressif's recommended P4 setting) are the only effective compute lever found: over the first 41 s of the same benchmark (sysmon samples every 340 ms), heavy-frame render time drops from p90 21 ms / max 22 ms to p90 15 ms / max 16 ms, the share of time spent at 30 FPS (VSYNC halving) falls from 25% to 13%, and the average FPS rises from 51.6 to 54.7; FreeRTOS run-time stats show the load split across both cores (core 0 ~35%, core 1 ~48%, versus 88% on core 0 single-threaded).

They used to livelock reproducibly at the benchmark's teardown; on 2026-08-30 the root cause was found and fixed, and **two draw threads are now the example default** (together with `CONFIG_LV_USE_CLIB_MALLOC=y`, see below). The root cause is the builtin LVGL memory pool being far too small (`CONFIG_LV_MEM_SIZE_KILOBYTES=64`): on this 262×928 panel under benchmark load the pool must hold the widget tree, hundreds of per-frame draw task structs (`lv_draw_task_t`, 100–300 B each) and 24 KB sub-layer buffers (`LV_DRAW_LAYER_SIMPLE_BUF_SIZE`) at once, and two draw threads double the peak of in-flight tasks and concurrent layers — the pool reliably runs dry about 41 s in. What happens next depends on which allocation fails first; all three modes were reproduced on hardware: ① a sub-layer buffer allocation fails → the task stays `WAITING` forever and draw dispatch spins in `lv_thread_sync_wait` / mutex give-take (watchdog register dumps symbolize into `lv_freertos.c`), fixed upstream by [lvgl/lvgl#10369](https://github.com/lvgl/lvgl/pull/10369) (merged into master 2026-08-03, lands in 9.6) which marks the task `FAILED` and drops it; ② with that patch cherry-picked onto 9.5 the livelock disappears but rendering degrades into continuously dropped tasks (a visibly broken screen); ③ soon after, `lv_draw_add_task()` fails to allocate the task struct itself and hits the default `LV_ASSERT_MALLOC` handler — `while(1);` — a silent freeze. So #10369 is defensive hardening only; **the real fix is giving LVGL enough memory**: `CONFIG_LV_USE_CLIB_MALLOC=y` routes `lv_malloc` to the IDF heap — the same TLSF algorithm (IDF's `multi_heap` is built on tlsf.h) with the same fragmentation behavior, but sized by all of internal SRAM (blocks > 16 KB may spill to PSRAM, threshold `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL`), at the cost of losing the isolation of a private pool; it is also the configuration Espressif's esp_lvgl_adapter README officially recommends. With it the benchmark runs to completion, prints its summary and triggers zero watchdogs, averaging 52 FPS across all scenes (two threads, LVGL logging enabled).
